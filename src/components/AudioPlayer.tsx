    ...
    if (isPlaying) {
      ...
      audio.play()
        ...
        .catch((err) => {
          ...
          // If play failed, synchronize the state back to paused
          if (isPlaying) {
            onTogglePlay();
          }
        });
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[990] flex items-center gap-3">
      ...
      <button
        ...
        className={`group relative flex items-center justify-center w-12 h-12 rounded-full overflow-hidden transition-all duration-500 border bg-black/30 backdrop-blur-lg shadow-[0_4px_30px_rgba(0,0,0,0.4)] ${
          isPlaying
